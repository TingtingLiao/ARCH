...
if __name__ == "__main__":
    ...
    fiter = HGFilter(opt)
    data_dir =  "/home/ting/Data/SURREAL/images/dataset-surreal"
    assert os.path.exists(data_dir)
    for idx, f in enumerate(os.listdir(data_dir)):

        image_name = f.split(".")[0] + ".jpg"
        img = implt.imread(os.path.join(data_dir, image_name))
        img = torch.FloatTensor(img).unsqueeze(0).permute(0, 3, 1, 2)
        im_feat_list, tmpx, normx = fiter(img)
        for feat in im_feat_list:
            print(feat.size())
        print(tmpx.size(), normx.size())
        break