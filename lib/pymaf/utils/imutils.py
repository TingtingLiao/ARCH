...
def process_image(img_file, det, mask_file=None, input_res=512):
    """Read image, do preprocessing and possibly crop it according to the bounding box.
    If there are bounding box annotations, use them to crop the image.
    If no bounding box is specified but openpose detections are available, use them to get the bounding box.
    """
    ...
    # TODO: use human_inst_seg for segmentation
    if mask_file is None:
        # remove background
        cv2.imwrite('/home/liaotingting/tmp/im.png', img_np[..., ::-1])
        os.system('cd ~/projects/CIHP_PGN &&'
                  'python test.py --data_dir /home/liaotingting/tmp')

        mask_file = '/home/liaotingting/tmp/MASK/im.png'
